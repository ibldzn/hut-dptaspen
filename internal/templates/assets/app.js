...
function validateCounts() {
  ...
  const availableRegular =
    state.pool.regular.tad.length + state.pool.regular.organik.length;
  ...
  if (availableRegular < remainingGrandDraws) {
    return `Data non-guaranteed kurang untuk grand prize. Minimal ${remainingGrandDraws}, tersedia ${availableRegular}.`;
  }
...
function drawFromGroup(group, remainingDraws) {
  if (!group || (group.tad.length === 0 && group.organik.length === 0)) {
    return null;
  }
  const mustTad = state.remainingTad === remainingDraws;
  const mustOrganik = state.remainingTad === 0;
  let pickTad = mustTad
    ? true
    : mustOrganik
      ? false
      : Math.random() < state.remainingTad / remainingDraws;
...
function drawDoorPrize(remainingDraws) {
  ...
  let group = pickGuaranteed ? state.pool.guaranteed : state.pool.regular;
  let usedGuaranteed = pickGuaranteed;
  let winner = drawFromGroup(group, remainingDraws);

  if (!winner) {
    group = pickGuaranteed ? state.pool.regular : state.pool.guaranteed;
    usedGuaranteed = !pickGuaranteed;
    winner = drawFromGroup(group, remainingDraws);
  }
...
function drawGrandPrize(remainingDraws) {
  const winner = drawFromGroup(state.pool.regular, remainingDraws);
  if (!winner) {
    return null;
  }
